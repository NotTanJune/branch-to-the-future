...
fn cli_defaults_are_cost_optimized_for_prototyping() {
    ...
    assert_eq!(cli.max_output_tokens, 2_500);
...
fn impact_parser_clamps_scores_and_rejects_unknown_enums() {
    let valid = r#"{
      ...
      "futures":[{"name":"Proper Architecture","description":"queue worker","complexity":"high","risk":"low","affected_files":["app/api/upload/route.ts"],"benefits":["robust"],"drawbacks":["more code"],"patch_plan":["add queue"],"test_plan":["status tests"]}],
...
fn markdown_export_contains_required_sections() {
    ...
    let path = export_markdown(
        ...
        &analysis,
        0,
        Some(std::path::Path::new("branch-futures-architecture.png")),
    )
    .unwrap();
    ...
    assert!(report.contains("branch-futures-architecture.png"));
}
...
async fn ai_client_sends_responses_request_and_retries_malformed_json() {
    ...
    let second_body = bodies_rx.recv().unwrap();
    assert!(second_body.contains("Previous response failed JSON parsing"));
}
...
#[tokio::test]
async fn ai_client_generates_architecture_image_file() {
    let (url, bodies_rx) = fake_openai_image_server();
    let client = OpenAiClient::with_base_url(
        "test-key".to_string(),
        url,
        "gpt-5.5".to_string(),
    ...
    let dir = tempfile::tempdir().unwrap();
    let image_path = dir.path().join("branch-futures-architecture.png");

    client
        .generate_architecture_diagram(&sample_analysis(), "add async resume parsing", &image_path)
        .await
        .unwrap();
    ...
    assert!(input.contains("label-light developer architecture blueprint"));
    assert!(input.contains("Leave generous empty space"));
}
...
fn fake_openai_image_server() -> (String, mpsc::Receiver<String>) {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let address = listener.local_addr().unwrap();
    let (tx, rx) = mpsc::channel();
    thread::spawn(move || {
...
fn valid_analysis_json() -> String {
    r#"{
      ...
      "risk_summary":["PII"],
      "tests_to_add":["returns job id"],
      "futures":[{"name":"Minimal Patch","description":"queue worker","complexity":"medium","risk":"medium","affected_files":[],"benefits":["small"],"drawbacks":["basic"],"patch_plan":["add status"],"test_plan":["status test"]}],
